...
class TransactionGroup {
  ...
  final double dayTotal; // Tổng tiền ngày đó (thu - chi)
...
final financialSummaryProvider = Provider<AsyncValue<FinancialSummary>>((ref) {
  ...
  return txAsync.when(
    ...
    data: (transactions) {
      ...
      return AsyncValue.data(FinancialSummary(
        totalIncome: income,
  ...
  return txAsync.when(
    ...
    data: (transactions) {
      ...
      final result = groups.entries.map((entry) {
        ...
        final dayTotal = txList.fold(0.0, (sum, t) {
  ...
  return txAsync.when(
    ...
    data: (transactions) {
      ...
      for (int i = 6; i >= 0; i--) {
        ...
        final dayTx = transactions.where((t) => _isSameDay(t.transactionDate, day));
